...
export function hide(storeCell, negate = false) {
    return (target, context) => {
        const m = readMeta(context);
        if (!m._declaredHides)
            m._declaredHides = new Map();
        const key = context.kind === "class" ? null : context.name;
        m._declaredHides.set(key, { cell: storeCell, negate });
        if (context.kind === "class")
            return target;
    };
}
export function classToggle(className, cell, negate = false) {
    return (target, context) => {
        const m = readMeta(context);
        if (!m._declaredClasses)
            m._declaredClasses = new Map();
        const key = context.kind === "class" ? null : context.name;
        m._declaredClasses.set(key, { className, cell, negate });
        if (context.kind === "class")
            return target;
    };
}
export function attr(attrName, cell, negate = false) {
    return (target, context) => {
        const m = readMeta(context);
        if (!m._declaredAttrs)
            m._declaredAttrs = new Map();
        const key = context.kind === "class" ? null : context.name;
        m._declaredAttrs.set(key, { attrName, cell, negate });
        if (context.kind === "class")
            return target;
    };
}
