import { subscribeToEntireStore } from "./walk.js";
export function reactFromAttr(attrVal, el, params) {
    if (!attrVal)
        return;
    ...
    parts.forEach(part => {
        ...
        const [relativePath, _type, attrName, ...exprArr] = part
            .split(":")
            .map(s => s.trim());
        if (!relativePath)
            return;
        const elAccessPath = elBaseKey
        ...
        switch (type) {
            ...
            case "style":
                if (attrName)
                    el.style[attrName] = result;
                break;
            case "class":
                if (result)
                    el.classList.add(attrName);
                else
                    el.classList.remove(attrName);
                break;
            case "textContent":
...
export function initReactiveHTML(store, root = document) {
    subscribeToEntireStore(store, params => {
        const elements = root.querySelectorAll(`[data-r], [data-r-0], [data-r-1]`);
        elements.forEach(el => {
