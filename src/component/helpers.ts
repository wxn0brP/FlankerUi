...
export function storeHide(
	element: HTMLElement,
	storeValue: ReactiveCell<boolean | any>,
) {
...
export function renderListNodes<T = any>(
	...
	container.innerHTML = "";
	if (!items?.length && ifElse) {
		typeof ifElse === "string"
			? (container.innerHTML = ifElse)
			: container.appendChild(ifElse);
		return;
	}
...
export function watchCheckbox(
	el: HTMLInputElement,
	store: ReactiveCell<boolean>,
	setFromElement = false,
) {
	if (setFromElement) store.set(el.checked);
	else el.checked = store.get();
	el.addEventListener("change", () => {
