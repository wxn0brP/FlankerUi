...
export function setFetchVQL(fn: typeof fetchVQLType) {
	fetchVQL = fn;
}
...
export function mountView<Extra extends Record<string, any> = {}>(
	...
	const el =
		typeof opts.selector === "string"
			? (document.querySelector(opts.selector) as HTMLElement)
			: opts.selector;
	...
	async function load(args: RenderArgs = {}) {
		...
		if (opts.queryFunction) data = await opts.queryFunction(assignArgs, args);
	...
	function render(data: RenderArgs, mode: TemplateDataMode = "replace") {
		...
		if (mode === "append" || mode === "prepend") {
			if (empty) {
				...
				if (
					el.innerHTML.trim() !== "" &&
					el.getAttribute("data-fu-empty") !== "true"
				) {
					return;
				}
				...
				if (el.getAttribute("data-fu-empty") === "true") {
					...
					el.innerHTML =
						mode === "append" ? el.innerHTML + res : res + el.innerHTML;
				}
